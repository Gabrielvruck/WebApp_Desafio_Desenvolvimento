...
namespace WebApp_Desafio_FrontEnd.ApiClients
{
    public abstract class BaseClient
    {
        ...
        private HttpWebRequest CreateWebRequest(ref string url, Dictionary<string, object> queries = null, Dictionary<string, object> headers = null)
        {
            ...
            if (url.Contains('?'))
            {
                throw new ArgumentException("URL já contém parâmetros. (?)", nameof(url));
            }
            ...
            if (queries != null && queries.Count > 0)
            {
                ...
                foreach (var query in queries)
                {
                    stringBuilder.Append($"{query.Key}={(query.Value ?? string.Empty)}&");
                }
            }

            var webRequest = (HttpWebRequest)WebRequest.Create(stringBuilder.ToString().TrimEnd('&'));
            ...
            if (headers != null && headers.Count > 0)
            {
                foreach (var header in headers)
                {
                    ...
                    {
                        Console.WriteLine($"Erro ao adicionar cabeçalho: {ex.Message}");
                    }
                }
            }
